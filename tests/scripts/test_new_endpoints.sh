...
# Start Server with MemoryStore and Auto-Auth
...
# Default Tenant ID is usually all zeros or logged
...
curl -s -X PUT "$BASE_URL/config/settings" \
    ...
    -d '{"allow_public_signup": false, "default_user_role": "TenantUser"}' | jq .
...
# Rotate implicitly rotates the CURRENT token
...
echo "---------------------------------------------------"
echo "3. Testing Federated Catalog (Mock)"
echo "POST /federated-catalogs/fed_cat/sync"
# This expects failure or mock response since we define no federation config in memory store usually
...
curl -s -X POST "$BASE_URL/warehouses" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "default_wh", "bucket": "test-bucket", "region": "us-east-1", "access_key": "x", "secret_key": "y", "storage_type": "s3"}' | jq . || true

echo "Creating Catalog..."
curl -v -X POST "$BASE_URL/catalogs" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "iceberg_cat", "type": "iceberg", "warehouse_name": "default_wh"}' | jq .

    -d '{"name": "iceberg_cat", "type": "iceberg", "warehouse_name": "default_wh"}' | jq .

echo "Creating Namespace..."
# Assuming iceberg REST spec or simple endpoint
curl -v -X POST "http://127.0.0.1:8080/v1/iceberg_cat/namespaces" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"namespace": ["sales"]}' | jq .

echo "List Namespaces (Flat)..."
curl -s -X GET "http://127.0.0.1:8080/v1/iceberg_cat/namespaces" -H "$AUTH_HEADER" -H "$TENANT_HEADER" | jq .

echo "GET /catalogs/iceberg_cat/namespaces/tree"
curl -s -X GET "$BASE_URL/catalogs/iceberg_cat/namespaces/tree" -H "$AUTH_HEADER" -H "$TENANT_HEADER" | jq .

echo "Creating 'main' branch..."
curl -v -X POST "$BASE_URL/branches" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "main", "catalog": "iceberg_cat", "branch_type": "experimental"}' | jq . || true

echo "Creating 'dev' branch from 'main'..."
curl -v -X POST "$BASE_URL/branches" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "dev", "catalog": "iceberg_cat", "from_branch": "main"}' | jq . || true

echo "---------------------------------------------------"
echo "5. Testing Branching (Rebase)"
echo "POST /branches/main/rebase"
# Fix payload: Name is required (ignored but required by struct deserialization)
curl -s -X POST "$BASE_URL/branches/main/rebase" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "ignored", "catalog": "iceberg_cat", "source_branch": "dev"}' | jq .
