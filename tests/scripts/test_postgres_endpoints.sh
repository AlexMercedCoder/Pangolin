...
# Note: Postgres requires unique constraints, and schema persistence. 
# Attempt creation, ignore error if exists, or use slightly random name? 
# Test environment is persistent if docker volume is reused.
# Let's clean up first or just assume new data? 
# For duplicate key errors, we'll see 500. Not ideal but functional for verifying API.
# Actually, let's use a random suffix for names to avoid collisions in persistent Postgres.
SUFFIX=$RANDOM
