...
async fn test_postgres_warehouse_crud() {
    ...
    // Get warehouse
    let retrieved = store
        .get_warehouse(tenant_id, warehouse_name.clone())
        .await
    ...
    // Delete warehouse
    store
        .delete_warehouse(tenant_id, warehouse_name.clone())
        .await
        .expect("Failed to delete warehouse");

    // Verify deletion
    let deleted = store
        .get_warehouse(tenant_id, warehouse_name)
        .await
        .expect("Failed to get warehouse after delete");
...
async fn test_postgres_catalog_crud() {
    ...
    // Verify deletion
    let deleted = store
        .get_catalog(tenant_id, catalog_name)
        .await
        .expect("Failed to get catalog after delete");
    assert!(deleted.is_none());
}
...
async fn test_postgres_namespace_operations() {
    ...
    store
        .create_namespace(tenant_id, &catalog_name, namespace.clone())
        .await
        .expect("Failed to create namespace");

    // Get namespace
    let retrieved = store
        .get_namespace(tenant_id, &catalog_name, namespace.name.clone())
        .await
    ...
    store
        .update_namespace_properties(tenant_id, &catalog_name, namespace.name.clone(), new_props)
        .await
        .expect("Failed to update namespace");

    // Delete namespace
    store
        .delete_namespace(tenant_id, &catalog_name, namespace.name.clone())
        .await
        .expect("Failed to delete namespace");

    // Verify deletion
    let deleted = store
        .get_namespace(tenant_id, &catalog_name, namespace.name)
        .await
        .expect("Failed to get namespace after delete");
    assert!(deleted.is_none());
}
...
async fn test_postgres_asset_operations() {
    ...
    // List assets
    let assets = store
        .list_assets(tenant_id, &catalog_name, None, namespace.name.clone(), None)
        .await
    ...
    let deleted = store
        ...
        .await
        .expect("Failed to get asset after delete");
    assert!(deleted.is_none());
}
...
async fn test_postgres_multi_tenant_isolation() {
    ...
    let delete_result = store
        .delete_warehouse(tenant2_id, "shared_name".to_string())
        .await;
...
async fn test_postgres_catalog_delete_cascade() {
    ...
    assert_eq!(
        store
            .list_assets(tenant_id, &catalog_name, None, vec!["db".to_string()], None)
            .await
            .unwrap()
            .len(),
        1
