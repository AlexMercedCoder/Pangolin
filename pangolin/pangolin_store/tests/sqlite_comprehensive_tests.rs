...
async fn test_sqlite_warehouse_crud() {
    ...
    store
        .create_warehouse(tenant.id, warehouse.clone())
        .await
        .expect("Failed to create warehouse");

    // Get warehouse
    let retrieved = store
        .get_warehouse(tenant.id, "test_warehouse".to_string())
        .await
    ...
    // Delete warehouse
    store
        .delete_warehouse(tenant.id, "test_warehouse".to_string())
        .await
        .expect("Failed to delete warehouse");
    let warehouses = store
        .list_warehouses(tenant.id, None)
        .await
        .expect("Failed to list warehouses after delete");
...
async fn test_sqlite_catalog_crud() {
    ...
    // Get catalog
    let retrieved = store
        .get_catalog(tenant.id, "test_catalog".to_string())
        .await
    ...
    let catalogs = store
        ...
        .await
        .expect("Failed to list catalogs");
    assert_eq!(catalogs.len(), 1);

    // Delete catalog
    store
        .delete_catalog(tenant.id, "test_catalog".to_string())
        .await
        .expect("Failed to delete catalog");
    let catalogs = store
        .list_catalogs(tenant.id, None)
        .await
        .expect("Failed to list catalogs after delete");
...
async fn test_sqlite_namespace_operations() {
    ...
    store
        .create_namespace(tenant.id, "test_catalog", namespace.clone())
        .await
    ...
    let retrieved = store
        .get_namespace(
            tenant.id,
            "test_catalog",
            vec!["db1".to_string(), "schema1".to_string()],
        )
        .await
    ...
    let namespaces = store
        .list_namespaces(tenant.id, "test_catalog", None, None)
        .await
    ...
    let updated = store
        ...
        .await
        .expect("Failed to get updated namespace");
    assert!(updated.is_some());
    assert_eq!(
        updated.unwrap().properties.get("key1"),
    ...
    let namespaces = store
        .list_namespaces(tenant.id, "test_catalog", None, None)
        .await
        .expect("Failed to list namespaces after delete");
...
async fn test_sqlite_multi_tenant_isolation() {
    ...
    store
        .create_tenant(tenant1.clone())
        .await
        .expect("Failed to create tenant1");
    store
        .create_tenant(tenant2.clone())
        .await
    ...
    store
        .create_catalog(tenant1.id, catalog1.clone())
        .await
        .expect("Failed to create catalog for tenant1");

    // Verify tenant2 cannot see tenant1's catalog
    let tenant2_catalogs = store
        .list_catalogs(tenant2.id, None)
        .await
        .expect("Failed to list catalogs for tenant2");
    assert_eq!(tenant2_catalogs.len(), 0);

    // Verify tenant1 can see their catalog
    let tenant1_catalogs = store
        .list_catalogs(tenant1.id, None)
        .await
        .expect("Failed to list catalogs for tenant1");
...
async fn test_sqlite_catalog_delete_cascade() {
    ...
    store
        .create_namespace(tenant.id, "test_catalog", ns.clone())
        .await
        .expect("Failed");
...
async fn test_sqlite_access_requests() {
    ...
    store
        .create_namespace(tenant.id, "def", namespace.clone())
        .await
    ...
    // 3. List Requests
    let list = store
        .list_access_requests(tenant.id, None)
        .await
...
async fn test_sqlite_list_user_permissions_aggregation() {
    ...
    let role_scope = PermissionScope::Catalog {
    ...
    let direct_scope = PermissionScope::Catalog {
        catalog_id: Uuid::new_v4(),
    };
    let mut direct_actions = HashSet::new();
    direct_actions.insert(Action::Write);
    let direct_perm = Permission::new(
        user_id,
        tenant_id,
        direct_scope.clone(),
        direct_actions,
    ...
    assert_eq!(
        aggregated_perms.len(),
        2,
        "Should have 2 permissions (1 direct, 1 from role)"
    );

    let has_direct = aggregated_perms
        .iter()
        .any(|p| p.scope == direct_scope && p.actions.contains(&Action::Write));
    let has_role_based = aggregated_perms
        .iter()
        .any(|p| p.scope == role_scope && p.actions.contains(&Action::Read));

    assert!(
        has_direct,
        "Aggregated permissions should include direct permission"
    );
    assert!(
        has_role_based,
        "Aggregated permissions should include role-based permission"
    );
}
