...
async fn test_token_revocation() {
    ...
    // Revoke the token
    store
        .revoke_token(token_id, expires_at, Some("User logout".to_string()))
        .await
        .unwrap();
...
async fn test_token_cleanup() {
    ...
    // Revoke tokens
    store
        .revoke_token(expired_token1, past, None)
        .await
        .unwrap();
    store
        .revoke_token(expired_token2, past, None)
        .await
        .unwrap();
...
async fn test_multiple_token_revocations() {
    ...
    for token_id in &tokens {
        store
            .revoke_token(*token_id, expires_at, Some(format!("Revoked {}", token_id)))
            .await
            .unwrap();
    }
