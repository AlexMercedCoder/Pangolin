...
async fn test_postgres_audit_log_filtering() {
    ...
    let store: PostgresStore = PostgresStore::new(&connection_string).await.expect("Failed to create PostgresStore");
    ...
    println!("\n✅ All PostgreSQL audit logging tests passed!");
}
...
async fn test_postgres_audit_log_time_filtering() {
    ...
    let old_logs = store.list_audit_events(tenant_id, Some(filter)).await.unwrap();
    assert_eq!(old_logs.len(), 2, "Should have 2 logs before one hour ago");

    println!("✓ PostgreSQL time filtering test passed");
}
