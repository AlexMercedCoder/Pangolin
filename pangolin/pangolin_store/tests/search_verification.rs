...
async fn test_memory_store_search() {
    ...
    store
        .upsert_business_metadata(BusinessMetadata {
            id: Uuid::new_v4(),
            asset_id,
    ...
    let results = store
        .search_assets(tenant_id, "", Some(vec!["missing".to_string()]))
        .await
        .unwrap();
    ...
    let results = store
        .search_assets(tenant_id, "revenue", None)
        .await
        .unwrap();
    ...
    let results = store
        .search_assets(tenant_id, "Revenue Data", None)
        .await
        .unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].0.name, "revenue_q1");
    ...
    let results = store
        .search_assets(tenant_id, "", Some(vec!["finance".to_string()]))
        .await
        .unwrap();
    assert_eq!(results.len(), 1);
