...
async fn run_store_compliance_tests<S: CatalogStore>(store: &S, store_name: &str) {
    ...
    store
        .create_catalog(tenant_id, catalog)
        .await
        .expect("create_catalog failed");
    ...
    store
        ...
        .await
        .expect(
            "update_metadata_location failed - Optimistic Locking/Metadata Location Regression!",
        );
    ...
    match store.write_file(&file_path, content.to_vec()).await {
        ...
        Err(e) => {
            println!(
                "Store {} does not support File IO or failed: {}",
                store_name, e
            );
...
async fn test_mongo_store_compliance() {
    ...
    let store = MongoStore::new(&db_url, &db_name)
        .await
