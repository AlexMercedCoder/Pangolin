...
async fn test_aws_static_vending_strategy() {
    ...
    let result = store
        .get_table_credentials("s3://mybucket/table")
        .await
        .unwrap();
    if let Credentials::Aws {
        access_key_id,
        secret_access_key,
        ..
    } = result
...
async fn test_aws_legacy_static_config() {
    ...
    let result = store
        .get_table_credentials("s3://legacybucket/table")
        .await
        .unwrap();
    if let Credentials::Aws {
        access_key_id,
...
async fn test_aws_legacy_sts_execution_attempt() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                !msg.contains("AWS STS vending deprecated"),
                "Legacy STS logic was blocked by deprecation error: {}",
                msg
            );
...
fn test_azure_path_parsing() {
    ...
    // Test abfs://
    let (container, path) =
        parse_azure_path("abfs://mycontainer@account.dfs.core.windows.net/path/to/blob").unwrap();
    assert_eq!(container, "mycontainer");
    assert_eq!(path, "path/to/blob");
}
...
async fn test_memory_store_vending_strategy() {
    ...
    #[cfg(feature = "azure")]
    {
        ...
        let result = store
            .get_table_credentials("az://testcontainer/table")
            .await;
        ...
        if let Err(e) = &result {
            println!(
                "Azure signer returned error (expected without credentials/network): {}",
                e
            );
            // Verify it's not "No warehouse found".
            assert!(
                e.to_string().contains("reqwest")
                    || e.to_string().contains("client")
                    || e.to_string().contains("SDK")
...
async fn test_s3_flat_key_support() {
    ...
    let result = store
        .get_table_credentials("s3://flatbucket/table")
        .await
        .unwrap();
    if let Credentials::Aws {
        access_key_id,
    ...
    let write_res = store
        .write_file("s3://flatbucket/preamble/test.txt", data)
        .await;
