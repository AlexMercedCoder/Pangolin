...
impl MemoryStore {
    pub(crate) async fn create_catalog_internal(
        &self,
        tenant_id: Uuid,
        catalog: Catalog,
    ) -> Result<()> {
    ...
    pub(crate) async fn get_catalog_internal(
        &self,
        tenant_id: Uuid,
        name: String,
    ...
    pub(crate) async fn search_catalogs_internal(
        &self,
        tenant_id: Uuid,
        query: &str,
    ) -> Result<Vec<Catalog>> {
    ...
    pub(crate) async fn list_catalogs_internal(
        &self,
        tenant_id: Uuid,
        pagination: Option<crate::PaginationParams>,
    ) -> Result<Vec<Catalog>> {
        let mut catalogs: Vec<Catalog> = self
            .catalogs
            .iter()
    ...
    pub(crate) async fn update_catalog_internal(
        &self,
        tenant_id: Uuid,
        name: String,
    ...
    pub(crate) async fn delete_catalog_internal(
        &self,
        tenant_id: Uuid,
        name: String,
    ) -> Result<()> {
        ...
        if self.catalogs.remove(&key).is_some() {
            ...
            // Remove Namespaces
            self.namespaces
                .retain(|k, _| !(k.0 == tenant_id && k.1 == name));
