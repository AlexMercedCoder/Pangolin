...
impl MemoryStore {
    pub(crate) async fn create_asset_internal(
        ...
        let mut branch_obj = self
            .get_branch_internal(tenant_id, catalog_name, branch_name.clone())
            .await?
        ...
        if !branch_obj.assets.contains(&asset_full_name) {
            ...
            self.create_branch_internal(tenant_id, catalog_name, branch_obj)
                .await?;
        }

        Ok(())
    }
    pub(crate) async fn get_asset_internal(
        &self,
        tenant_id: Uuid,
        catalog_name: &str,
        branch: Option<String>,
    ...
    pub(crate) async fn get_asset_by_id_internal(
        &self,
        tenant_id: Uuid,
        asset_id: Uuid,
    ...
    pub(crate) async fn rename_asset_internal(
        &self,
        tenant_id: Uuid,
        catalog_name: &str,
        branch: Option<String>,
    ...
    pub(crate) async fn search_assets_internal(
        &self,
        tenant_id: Uuid,
        query: &str,
            ...
            if *tid == tenant_id {
                ...
                if (name_matches || description_matches) && tags_match {
                    let namespace: Vec<String> =
                        ns_str.split('\x1F').map(|s| s.to_string()).collect();
    ...
    pub(crate) async fn copy_assets_bulk_internal(
        ...
        let mut branch_obj = self
            .get_branch_internal(tenant_id, catalog_name, dest_branch.to_string())
            .await?
            .unwrap_or_else(|| Branch {
