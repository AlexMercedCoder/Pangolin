...
impl MemoryStore {
    pub(crate) async fn store_token_internal(
        &self,
        token: pangolin_core::token::TokenInfo,
    ) -> Result<()> {
        self.active_tokens.insert(token.id, token);
        Ok(())
    }
    pub(crate) async fn list_active_tokens_internal(
        &self,
        tenant_id: Uuid,
        user_id: Option<Uuid>,
    ...
    pub(crate) async fn revoke_token_internal(
        &self,
        token_id: Uuid,
