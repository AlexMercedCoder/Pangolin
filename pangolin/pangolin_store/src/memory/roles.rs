...
impl MemoryStore {
    pub(crate) async fn create_role_internal(
        &self,
        role: pangolin_core::permission::Role,
    ) -> Result<()> {
        self.roles.insert(role.id, role);
        Ok(())
    }
    pub(crate) async fn get_role_internal(
        &self,
        role_id: Uuid,
    ) -> Result<Option<pangolin_core::permission::Role>> {
        Ok(self.roles.get(&role_id).map(|r| r.value().clone()))
    }
    pub(crate) async fn list_roles_internal(
        &self,
        tenant_id: Uuid,
    ...
    pub(crate) async fn assign_role_internal(
        &self,
        user_role: pangolin_core::permission::UserRole,
    ) -> Result<()> {
