...
impl MemoryStore {
    pub(crate) async fn get_metadata_location_internal(
        &self,
        tenant_id: Uuid,
        catalog_name: &str,
    ...
    pub(crate) async fn update_metadata_location_internal(
        &self,
        tenant_id: Uuid,
        catalog_name: &str,
        branch: Option<String>,
        ...
        if let Some(mut asset) = self.assets.get_mut(&key) {
            ...
            asset
                .properties
                .insert("metadata_location".to_string(), new_location);
            Ok(())
        } else {
    ...
    pub(crate) async fn write_file_internal(&self, location: &str, content: Vec<u8>) -> Result<()> {
        ...
        if let Some(warehouse) = self.get_warehouse_for_location(location) {
                ...
                || location.starts_with("gs://")
            {
                ...
                let cache_key =
                    self.get_object_store_cache_key(&warehouse.storage_config, location);
    ...
    pub(crate) async fn expire_snapshots_internal(
        &self,
        _tenant_id: Uuid,
        _catalog_name: &str,
    ...
    pub(crate) async fn remove_orphan_files_internal(
        &self,
        _tenant_id: Uuid,
        _catalog_name: &str,
        _branch: Option<String>,
        ...
        if let Some(warehouse) = self.get_warehouse_for_location(location) {
                ...
                || location.starts_with("gs://")
            {
                ...
                match store.get(&path).await {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "Failed to read from object store for {}, falling back to memory: {}",
                            location,
                            e
                        );
                    }
    ...
    pub fn get_warehouse_for_location(
        &self,
        location: &str,
    ) -> Option<pangolin_core::model::Warehouse> {
        let warehouses_map: Vec<pangolin_core::model::Warehouse> = self
            .warehouses
            .iter()
        ...
        for warehouse in warehouses_map {
            if let Some(bucket) = warehouse
                .storage_config
                .get("s3.bucket")
                .or_else(|| warehouse.storage_config.get("bucket"))
            {
    ...
    fn get_object_store_cache_key(
        ...
        let access_key = config
            ...
            .or_else(|| config.get("gcp.service-account"))
            .map(|s| s.as_str())
            .unwrap_or("");
        let region = config
            .get("s3.region")
