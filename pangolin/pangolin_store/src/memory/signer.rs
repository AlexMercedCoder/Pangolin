...
impl MemoryStore {
    pub(crate) async fn get_table_credentials_internal(
        ...
        let warehouses_map: Vec<Warehouse> = self
            .warehouses
            .iter()
        ...
        match &warehouse.vending_strategy {
            ...
            Some(VendingStrategy::AwsStatic {
                access_key_id,
                secret_access_key,
            }) => Ok(Credentials::Aws {
            ...
            Some(VendingStrategy::AzureSas {
                account_name,
                account_key,
            }) => {
                #[cfg(feature = "azure")]
                {
                    let signer = crate::azure_signer::AzureSigner::new(
                        account_name.clone(),
                        account_key.clone(),
                    );
            ...
            Some(VendingStrategy::GcpDownscoped {
                service_account_email,
                private_key,
            }) => {
                #[cfg(feature = "gcp")]
                {
                    let signer = crate::gcp_signer::GcpSigner::new(
            ...
            None => {
                ...
                if warehouse.use_sts {
                    let client =
                        crate::aws_utils::create_sts_client(&warehouse.storage_config).await?;
                    let role_arn = warehouse
                        .storage_config
                        .get("s3.role-arn")
                        .map(|s| s.as_str());

                    crate::aws_utils::assume_role(&client, role_arn, None, "pangolin-memory-legacy")
                        .await
                } else {
