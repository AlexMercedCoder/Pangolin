...
impl SqliteStore {
    ...
    pub async fn write_file(&self, path: &str, data: Vec<u8>) -> Result<()> {
        ...
        if let Some(rest) = path.strip_prefix("file://") {
            ...
            Err(anyhow::anyhow!(
                "Only s3:// and file:// paths are supported in SQLite store"
            ))
        }
    }
    ...
    pub fn get_object_store_cache_key(
        &self,
    ...
    pub async fn get_warehouse_for_location(&self, location: &str) -> Result<Option<Warehouse>> {
        let rows = sqlx::query(
            "SELECT id, name, tenant_id, use_sts, storage_config, vending_strategy FROM warehouses",
        )
        .fetch_all(&self.pool)
        .await?;

        tracing::info!(
            "DEBUG_SQLITE: get_warehouse_for_location checking {} warehouses for location: {}",
            rows.len(),
            location
        );
        ...
        for row in rows {
            ...
            tracing::info!(
                "DEBUG_SQLITE: Checking warehouse ID: {:?}, config: {}",
                row.get::<String, _>("id"),
                config_str
            );

            if let Ok(config) =
                serde_json::from_str::<std::collections::HashMap<String, String>>(&config_str)
            {
                ...
                let s3_match = config
                    ...
                    .unwrap_or(false);
                let azure_match = config
                    .get("azure.container")
                    .map(|c| location.contains(c))
                    .unwrap_or(false);
                let gcp_match = config
                    .get("gcp.bucket")
                    .map(|b| location.contains(b))
                    .unwrap_or(false);
                ...
                if s3_match || azure_match || gcp_match {
                    ...
                    let vending_strategy =
                        vending_strategy_str.and_then(|s| serde_json::from_str(&s).ok());
                    ...
                    tracing::info!(
                        "DEBUG_SQLITE: No match. s3_match={}, azure_match={}, gcp_match={}",
                        s3_match,
                        azure_match,
                        gcp_match
                    );
