...
impl SqliteStore {
    ...
    pub async fn get_role(&self, role_id: Uuid) -> Result<Option<Role>> {
        ...
        if let Some(row) = row {
            Ok(Some(Role {
                ...
                created_at: chrono::DateTime::from_timestamp_millis(row.get("created_at"))
                    .unwrap_or_default(),
                updated_at: chrono::DateTime::from_timestamp_millis(row.get("updated_at"))
                    .unwrap_or_default(),
            }))
        } else {
            Ok(None)
    ...
    pub async fn get_user_roles(&self, user_id: Uuid) -> Result<Vec<UserRoleAssignment>> {
        let rows = sqlx::query(
            "SELECT user_id, role_id, assigned_by, assigned_at FROM user_roles WHERE user_id = ?",
        )
        .bind(user_id.to_string())
        .fetch_all(&self.pool)
        .await?;
        ...
        for row in rows {
            roles.push(UserRoleAssignment {
                ...
                assigned_at: chrono::DateTime::from_timestamp_millis(row.get("assigned_at"))
                    .unwrap_or_default(),
            });
        }
        Ok(roles)
