...
impl CatalogStore for MongoStore {
    ...
    async fn list_namespaces(&self, tenant_id: Uuid, catalog_name: &str, parent: Option<String>) -> Result<Vec<Namespace>> {
        self.list_namespaces(tenant_id, catalog_name, parent).await
    }
    ...
    // Token Management
    async fn list_active_tokens(&self, tenant_id: Uuid, user_id: Uuid) -> Result<Vec<TokenInfo>> {
        self.list_active_tokens(tenant_id, user_id).await
    }
    ...
    async fn list_audit_events(&self, tenant_id: Uuid, filter: Option<pangolin_core::audit::AuditLogFilter>) -> Result<Vec<pangolin_core::audit::AuditLogEntry>> {
        self.list_audit_events(tenant_id, filter.unwrap_or_default()).await
    }
    ...
    async fn count_audit_events(&self, tenant_id: Uuid, filter: Option<pangolin_core::audit::AuditLogFilter>) -> Result<usize> {
        self.count_audit_events(tenant_id, filter.unwrap_or_default()).await
    }
    ...
    async fn list_service_users(&self, tenant_id: Uuid) -> Result<Vec<ServiceUser>> {
        self.list_service_users(tenant_id).await
    }
    ...
    async fn list_merge_operations(&self, tenant_id: Uuid, catalog_name: &str) -> Result<Vec<MergeOperation>> {
        self.list_merge_operations(tenant_id, catalog_name).await
    }
    ...
    async fn list_merge_conflicts(&self, merge_op_id: Uuid) -> Result<Vec<MergeConflict>> {
        self.list_merge_conflicts(merge_op_id).await
    }
