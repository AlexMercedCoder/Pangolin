...
use pangolin_core::business_metadata::{AccessRequest, BusinessMetadata};
...
impl CatalogStore for MongoStore {
    ...
    async fn log_audit_event(
        &self,
        _tenant_id: Uuid,
        entry: pangolin_core::audit::AuditLogEntry,
    ) -> Result<()> {
    ...
    async fn update_service_user(
        ...
        let _ = self
            .update_service_user(id, name, description, None, active)
            .await?;
        Ok(())
    }
    ...
    ) -> Result<Vec<(Asset, Option<BusinessMetadata>, String, Vec<String>)>> {
    ...
    async fn get_federated_catalog_stats(
        ...
        Ok(self
            .get_sync_stats(tenant_id, catalog_name)
            .await?
