...
impl MongoStore {
    ...
    pub async fn read_file_uncached(&self, path: &str) -> Result<Vec<u8>> {
        ...
        if let Some(rest) = path.strip_prefix("s3://") {
            ...
            Err(anyhow::anyhow!(
                "Only s3:// paths are supported in Mongo store"
            ))
        }
    }
    ...
    pub async fn write_file(&self, path: &str, data: Vec<u8>) -> Result<()> {
        ...
        if let Some(rest) = path.strip_prefix("s3://") {
            ...
            Err(anyhow::anyhow!(
                "Only s3:// paths are supported for write in Mongo store"
            ))
        }
    }
    ...
    pub(crate) async fn get_warehouse_for_location(
        &self,
        location: &str,
        ...
        let warehouses: Vec<Warehouse> =
            cursor.try_collect().await.map_err(|e| anyhow::anyhow!(e))?;

        for warehouse in warehouses {
            let s3_match = warehouse
                .storage_config
                .get("s3.bucket")
                .or_else(|| warehouse.storage_config.get("bucket"))
                .map(|b| location.contains(b))
                .unwrap_or(false);
            let azure_match = warehouse
                .storage_config
                .get("azure.container")
                .map(|c| location.contains(c))
                .unwrap_or(false);
            let gcp_match = warehouse
                .storage_config
                .get("gcp.bucket")
                .map(|b| location.contains(b))
                .unwrap_or(false);
    ...
    pub(crate) fn get_object_store_cache_key(
        &self,
