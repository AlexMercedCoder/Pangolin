...
echo "=== MongoStore Performance Optimization Live Test ==="
echo ""
...
until docker exec mongo mongosh --eval "db.runCommand('ping').ok" --username root --password example --authenticationDatabase admin > /dev/null 2>&1; do
    sleep 1
done
echo "Mongo is ready!"

# 3. Start API with Mongo backend
echo ""
echo "3. Starting Pangolin API (Mongo backend)..."
export CHECK_SVC=true
