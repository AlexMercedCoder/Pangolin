...
echo "=== SQLiteStore Performance Optimization Live Test ==="
echo ""
...
# 3. Start API with SQLite backend
echo ""
echo "3. Starting Pangolin API (SQLite backend)..."
export STORE_TYPE=sqlite
export DATABASE_URL="sqlite:///tmp/pangolin_test.db"
...
# Extract token from logs (from the PyIceberg configuration snippet)
...
print("  ✓ Files persisted to MinIO - Verified via mc ls")
