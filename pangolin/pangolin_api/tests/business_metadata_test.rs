...
async fn test_business_metadata_flow() {
    ...
    let app = Router::new()
        .route(
            ...
            post(add_business_metadata).get(get_business_metadata),
        )
    ...
    let user = User::new_root("admin".to_string(), "admin@example.com".to_string(), hash);
    ...
    store
        .create_namespace(tenant_id, "default", namespace)
        .await
        .unwrap();
