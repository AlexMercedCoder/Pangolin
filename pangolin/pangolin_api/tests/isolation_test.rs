...
mod tests {
    ...
    use pangolin_api::warehouse_handlers::{
        create_warehouse, list_warehouses, CreateWarehouseRequest, WarehouseResponse,
    };
    ...
    async fn test_warehouse_isolation() {
        ...
        let create_res: Response = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_string(&CreateWarehouseRequest {
        ...
        let body_b = axum::body::to_bytes(list_b_res.into_body(), usize::MAX)
            .await
            .unwrap();
        ...
        assert_eq!(
            list_b.len(),
            0,
            "User B should not see Tenant A's warehouse"
        );
    }
    ...
    async fn test_catalog_isolation() {
        ...
        let body_bytes_a = axum::body::to_bytes(login_a_res.into_body(), usize::MAX)
            .await
            .unwrap();
        let token_a = serde_json::from_slice::<Value>(&body_bytes_a).unwrap()["token"]
            .as_str()
            .unwrap()
            .to_string();
        ...
        let body_bytes_b = axum::body::to_bytes(login_b_res.into_body(), usize::MAX)
            .await
            .unwrap();
        let token_b = serde_json::from_slice::<Value>(&body_bytes_b).unwrap()["token"]
            .as_str()
            .unwrap()
            .to_string();
        ...
        let list_a = app
            ...
            .await
            .unwrap();
        let body_a = axum::body::to_bytes(list_a.into_body(), usize::MAX)
            .await
            .unwrap();
        ...
        let list_b = app
            ...
            .await
            .unwrap();
        let body_b = axum::body::to_bytes(list_b.into_body(), usize::MAX)
            .await
            .unwrap();
