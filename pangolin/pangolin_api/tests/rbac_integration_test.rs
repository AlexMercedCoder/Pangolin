...
async fn test_tenant_user_create_namespace_denied() {
    ...
    let stored_user = store
        .get_user_by_username("tenant_user")
        .await
        .unwrap()
        .expect("User not found in store");
    assert_eq!(stored_user.username, "tenant_user");
    let stored_hash = stored_user
        .password_hash
        .as_ref()
        .expect("No password hash");
    let verify_result = pangolin_api::auth_middleware::verify_password(password, stored_hash)
