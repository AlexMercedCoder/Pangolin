...
mod iceberg_endpoint_tests {
    ...
    use std::sync::Arc;
    use pangolin_core::model::{Tenant, Warehouse};
    use uuid::Uuid;

    async fn setup_test_env() -> (Arc<dyn pangolin_store::CatalogStore + Send + Sync>, Uuid) {
    ...
    async fn test_get_config_without_warehouse_param() {
        let (store, _) = setup_test_env().await;
        let app = app(store);
    ...
    async fn test_list_namespaces_empty() {
        let (store, tenant_id) = setup_test_env().await;
        let app = app(store);
