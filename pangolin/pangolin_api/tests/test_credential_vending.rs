...
async fn test_aws_sts_vending_strategy() {
    ...
    let retrieved = store
        .get_warehouse(tenant_id, "sts_warehouse".to_string())
        .await
        .unwrap();
...
async fn test_aws_static_vending_strategy() {
    ...
    // Verify warehouse configuration
    let retrieved = store
        .get_warehouse(tenant_id, "static_warehouse".to_string())
        .await
        .unwrap();
    assert!(retrieved.is_some());
    let wh = retrieved.unwrap();
...
async fn test_bucket_resolution_from_warehouse() {
    ...
    let retrieved = store
        .get_warehouse(tenant_id, "bucket_warehouse".to_string())
        .await
        .unwrap();
    assert!(retrieved.is_some());
    let wh = retrieved.unwrap();
    assert_eq!(
        wh.storage_config.get("s3.bucket"),
        Some(&"my-specific-bucket".to_string())
    );
}
