...
async fn test_e2e_multi_cloud_no_regression() {
    ...
    let s3_result = s3_signer
        .generate_credentials("path", &["read".to_string()], chrono::Duration::hours(1))
        .await;
    let azure_result = azure_signer
        .generate_credentials("path", &["read".to_string()], chrono::Duration::hours(1))
        .await;
    let gcp_result = gcp_signer
        .generate_credentials("path", &["read".to_string()], chrono::Duration::hours(1))
        .await;
...
async fn test_e2e_s3_sts_mode() {
    ...
    if result.is_ok() {
        ...
        assert!(
            creds.config.contains_key("credential-type")
        ...
        assert!(
            err.to_string().contains("credentials") || err.to_string().contains("not configured")
        );
    }
}
