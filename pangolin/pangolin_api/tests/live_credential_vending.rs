...
async fn test_live_minio_static_credentials() {
    ...
    assert!(
        result.is_ok(),
        "Failed to vend MinIO credentials: {:?}",
        result.err()
    );
    ...
    assert_eq!(
        creds.config.get("s3.endpoint").unwrap(),
        "http://localhost:9000"
    );
...
async fn test_live_azurite_credentials() {
    ...
    storage_config.insert(
        "endpoint".to_string(),
        "http://localhost:10000/devstoreaccount1".to_string(),
    );
    ...
    assert!(
        result.is_ok(),
        "Failed to vend Azurite credentials: {:?}",
        result.err()
    );
    let creds = result.unwrap();
    ...
    assert_eq!(
        creds.prefix,
        "abfss://test-container@devstoreaccount1.dfs.core.windows.net/"
    );
    ...
    println!(
        "   Account: {}",
        creds.config.get("adls.account-name").unwrap()
    );
}
...
async fn test_live_fake_gcs_credentials() {
    ...
    assert!(
        result.is_ok(),
        "Failed to vend fake-gcs credentials: {:?}",
        result.err()
    );
    let creds = result.unwrap();
    ...
    println!(
        "   Project: {}",
        creds.config.get("gcp-project-id").unwrap()
    );
}
