...
fn test_static_credentials_response_format() {
    ...
    let access_key = warehouse
        .storage_config
        .get("access_key_id")
        .cloned()
        .unwrap();
    let secret_key = warehouse
        .storage_config
    ...
    config.insert(
        ...
        warehouse.storage_config.get("endpoint").cloned().unwrap(),
    );
    config.insert(
        "s3.region".to_string(),
        warehouse.storage_config.get("region").cloned().unwrap(),
    );
    ...
    assert_eq!(
        response.storage_credentials[0]
            .config
            .get("access-key")
            .unwrap(),
        "test_access_key"
...
fn test_azure_credentials_response_format() {
    ...
    let mut config = HashMap::new();
    let account_name = warehouse
        .storage_config
        .get("account_name")
        .cloned()
        .unwrap();
    let account_key = warehouse
        .storage_config
        .get("account_key")
        .cloned()
        .unwrap();
    ...
    assert_eq!(response.storage_credentials.len(), 1);
    assert!(response.storage_credentials[0]
        .prefix
        .starts_with("abfss://"));
    assert_eq!(
        response.storage_credentials[0]
...
fn test_gcs_credentials_response_format() {
    ...
    let service_account_key = warehouse
        .storage_config
...
fn test_sts_credentials_response_format() {
    ...
    assert!(response.storage_credentials[0]
        ...
        .get("access-key")
        .unwrap()
        .contains("STS_ACCESS_KEY"));
    assert_eq!(
        response.storage_credentials[0]
            .config
            .get("secret-key")
            .unwrap(),
        "STS_SECRET_KEY_PLACEHOLDER"
    );
    assert_eq!(
        response.storage_credentials[0]
            .config
...
fn test_table_response_includes_credentials() {
    ...
    assert_eq!(
        config.get("s3.secret-access-key").unwrap(),
        "test_secret_key"
    );
...
async fn test_assume_role_aws_placeholder() {
    ...
    let result: Result<(String, String, String, String), _> =
        pangolin_api::signing_handlers::assume_role_aws(
...
async fn test_get_azure_token_placeholder() {
    ...
    let result: Result<String, _> = pangolin_api::signing_handlers::get_azure_token(
        "tenant-id-123",
        "client-id-456",
        "client-secret-789",
    )
...
async fn test_get_gcp_token_placeholder() {
    ...
    let result: Result<String, _> =
        pangolin_api::signing_handlers::get_gcp_token(service_account_json).await;
