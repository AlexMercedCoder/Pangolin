...
mod tests {
    ...
    async fn test_iceberg_audit_logs() {
        ...
        let t_res = app
            ...
            .await
            .unwrap();
        let t_id_str = serde_json::from_slice::<serde_json::Value>(
            &to_bytes(t_res.into_body(), usize::MAX).await.unwrap(),
        )
        .unwrap()["id"]
            .as_str()
            .unwrap()
        ...
        app.clone()
            .oneshot(
                Request::builder()
                    ...
                    .header("Authorization", format!("Bearer {}", user_token))
                    .header("Content-Type", "application/json")
                    .body(Body::from(json!({"namespace": ["ns1"]}).to_string()))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        app.clone()
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri("/v1/cat/v1/namespaces/ns1/tables")
                    .header("Authorization", format!("Bearer {}", user_token))
                    .header("Content-Type", "application/json")
                    .body(Body::from(
                        json!({
        ...
        let create_table_log = logs
            .iter()
            .find(|l| l.action == AuditAction::CreateTable)
            .expect("create_table log missing");
        assert_eq!(create_table_log.username, "audit_user");

        // Iceberg "delete_table" logs as "DropTable" in legacy_new
        let delete_table_log = logs
            .iter()
            .find(|l| l.action == AuditAction::DropTable)
            .expect("drop_table log missing");
        assert_eq!(delete_table_log.username, "audit_user");

        let create_ns_log = logs
            .iter()
            .find(|l| l.action == AuditAction::CreateNamespace)
            .expect("create_namespace log missing");
