...
async fn main() {
    ...
    let storage_type =
        std::env::var("PANGOLIN_STORAGE_TYPE").unwrap_or_else(|_| "memory".to_string());

    let store: Arc<dyn CatalogStore + Send + Sync> = if let Ok(db_url) =
        std::env::var("DATABASE_URL")
    {
        if db_url.starts_with("postgresql://") || db_url.starts_with("postgres://") {
            tracing::info!("Using PostgreSQL storage backend");
            Arc::new(
                PostgresStore::new(&db_url)
                    .await
    ...
    if no_auth_enabled || seed_admin_enabled {
        ...
        let admin_password =
            std::env::var("PANGOLIN_ADMIN_PASSWORD").unwrap_or_else(|_| "password123".to_string());
        let jwt_secret = std::env::var("PANGOLIN_JWT_SECRET")
