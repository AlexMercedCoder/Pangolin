    ...
    } = scope
    {
        if let Ok(Some(metadata)) = store.get_business_metadata(*asset_id).await {
            for tag in metadata.tags {
                ...
                // 1. Direct
                for perm in &direct_perms {
                    if perm.scope.covers(&tag_scope)
                ...
                for user_role in &user_roles {
                    if let Ok(Some(role)) = store.get_role(user_role.role_id).await {
                        for grant in role.permissions {
                            if grant.scope.covers(&tag_scope)
                                && grant.actions.iter().any(|a| a.implies(action))
                            {
                                return Ok(true);
                            }
                        }
...
mod tests {
    use super::*;
    use pangolin_core::permission::{Permission, Role, UserRole as UserRoleStruct};
    ...
    async fn test_rbac_check() {
        ...
        let scope_ns = PermissionScope::Namespace {
            catalog_id,
        ...
        assert!(!check_permission(&store, &session, &read_action, &scope_ns)
            .await
            .unwrap());
        ...
        assert!(
            check_permission(&store, &session, &read_action, &scope_asset)
                .await
                .unwrap()
        );
        ...
        assert!(
            !check_permission(&store, &session, &write_action, &scope_ns)
                .await
                .unwrap()
        );
        ...
        assert!(check_permission(&store, &session, &write_action, &scope_ns)
            .await
            .unwrap());
    }
    ...
    async fn test_branch_and_tag_permissions() {
        ...
        let scope_branch = PermissionScope::Catalog { catalog_id };

        // Fail initially
        assert!(
            !check_permission(&store, &session, &branch_action, &scope_branch)
                .await
                .unwrap()
        );
        ...
        let tag_scope = PermissionScope::Tag {
