...
# Clean previous runs
rm -f server.log test_sqlite.db

# Start Server with SqliteStore
...
curl -s -X POST "http://127.0.0.1:8080/v1/iceberg_cat/namespaces" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
...
curl -s -X POST "$BASE_URL/branches" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "main", "catalog": "iceberg_cat", "branch_type": "experimental"}' | jq .

echo "Creating 'dev' branch from 'main'..."
curl -s -X POST "$BASE_URL/branches" \
    -H "$AUTH_HEADER" -H "$TENANT_HEADER" \
    -H "Content-Type: application/json" \
    -d '{"name": "dev", "catalog": "iceberg_cat", "from_branch": "main"}' | jq .
