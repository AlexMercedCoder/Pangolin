                        ...
                        match UserCli::try_parse_from(full_args) {
                            Ok(cli) => {
                                match cli.command {
                                    ...
                                    UserCommand::ListCatalogs => {
                                        if let Err(e) = handlers::handle_list_catalogs(&client).await {
                                            eprintln!("Error: {}", e);
                                        }
                                    },
                                    ...
                                    UserCommand::ListTags { catalog } => {
                                         if let Err(e) = handlers::handle_list_tags(&client, catalog).await { eprintln!("Error: {}", e); }
                                    },
                                    ...
                                    UserCommand::ListRequests => {
                                        if let Err(e) = handlers::handle_list_requests(&client).await { eprintln!("Error: {}", e); }
                                    },
