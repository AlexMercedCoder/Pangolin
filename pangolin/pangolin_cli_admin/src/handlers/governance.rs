...
async fn resolve_scope(
    ...
    if parts.len() != 2 {
        return Err(CliError::ApiError(
            "Invalid resource format. Use type:name (e.g. catalog:mycat, namespace:mycat.myns)"
                .to_string(),
        ));
    }
    let type_ = parts[0];
    ...
    match type_ {
        ...
        "namespace" => {
            ...
            if ns_parts.len() < 2 {
                return Err(CliError::ApiError(
                    "Invalid namespace format. Use catalog.namespace".to_string(),
                ));
            }
            ...
            if let Some((cat_name, rest)) = path.split_once('.') {
                if let Some((ns_name, tbl_name)) = rest.rsplit_once('.') {
                    ...
                    let catalogs: Vec<Value> = res
                        ...
                        .await
                        .map_err(|e| CliError::ApiError(e.to_string()))?;
                    let cat_id = if let Some(c) = catalogs
                        .iter()
                        .find(|c| c["name"].as_str() == Some(cat_name))
                    {
                        ...
                        .and_then(|v| v.as_str())
                    {
                        ...
                        Err(CliError::ApiError(
                            "Could not find table UUID in response".to_string(),
                        ))
                    }
                } else {
                    Err(CliError::ApiError(
                        "Invalid table format. Use catalog.namespace.table".to_string(),
                    ))
                }
            } else {
        ...
        _ => Err(CliError::ApiError(format!(
...
pub async fn handle_grant_permission(
    ...
    let actions: Vec<String> = action.split(',').map(|s| s.trim().to_lowercase()).collect();
...
pub async fn handle_assign_role(
    ...
    let res = client
        .post(&format!("/api/v1/users/{}/roles", user_id), &payload)
        .await?;
    if !res.status().is_success() {
...
pub async fn handle_revoke_user_role(
    client: &PangolinClient,
    user_id: String,
    role_id: String,
) -> Result<(), CliError> {
    let res = client
        .delete(&format!("/api/v1/users/{}/roles/{}", user_id, role_id))
        .await?;
    if !res.status().is_success() {
        return Err(CliError::ApiError(format!(
...
pub async fn handle_get_metadata(
    ...
    let res = client
        .get(&format!("/api/v1/assets/{}/metadata", entity_id))
        .await?;
    if !res.status().is_success() {
        return Err(CliError::ApiError(format!("Error: {}", res.status())));
    }
...
pub async fn handle_set_metadata(
    ...
    let description = current_metadata
        .get("description")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let tags = current_metadata
        .get("tags")
    ...
    let mut properties = current_metadata
        .get("properties")
        .cloned()
        .unwrap_or(serde_json::json!({}));
    ...
    let res = client
        .post(&format!("/api/v1/assets/{}/metadata", entity_id), &payload)
        .await?;
    if !res.status().is_success() {
        let s = res.status();
        let t = res.text().await.unwrap_or_default();
...
pub async fn handle_delete_metadata(
    client: &PangolinClient,
    asset_id: String,
) -> Result<(), CliError> {
    let res = client
        .delete(&format!("/api/v1/business-metadata/{}", asset_id))
        .await?;
    if !res.status().is_success() {
        return Err(CliError::ApiError(format!(
...
pub async fn handle_list_access_requests(
    client: &PangolinClient,
    limit: Option<usize>,
    offset: Option<usize>,
) -> Result<(), CliError> {
...
pub async fn handle_update_access_request(
    ...
    let res = client
        .put(&format!("/api/v1/access-requests/{}", id), &payload)
        .await?;
    if !res.status().is_success() {
        return Err(CliError::ApiError(format!(
