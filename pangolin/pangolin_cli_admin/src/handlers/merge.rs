...
pub async fn handle_list_merge_operations(
    ...
    if let Some(tid) = &client.config.tenant_id {
        ...
        let rows = items
            ...
            .collect();
        print_table(
            vec!["ID", "Source", "Target", "Status", "Initiated At"],
            rows,
        );
    } else {
...
pub async fn handle_get_merge_operation(
    client: &PangolinClient,
    id: String,
) -> Result<(), CliError> {
    ...
    if conflicts > 0 {
        println!(
            "\nUse 'pangolin-admin list-merge-conflicts --id {}' to view conflicts",
            id
        );
    }
...
pub async fn handle_list_merge_conflicts(
    client: &PangolinClient,
    id: String,
    limit: Option<usize>,
    offset: Option<usize>,
...
pub async fn handle_complete_merge_operation(
    client: &PangolinClient,
    id: String,
) -> Result<(), CliError> {
    let res = client
...
pub async fn handle_abort_merge_operation(
    client: &PangolinClient,
    id: String,
) -> Result<(), CliError> {
    let res = client
