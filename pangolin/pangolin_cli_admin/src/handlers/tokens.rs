...
pub async fn handle_revoke_token_by_id(
...
pub async fn handle_list_user_tokens(
    client: &PangolinClient,
    user_id: String,
    limit: Option<usize>,
    offset: Option<usize>,
    ...
    let path = if q.is_empty() {
        format!("/api/v1/users/{}/tokens", user_id)
    } else {
        format!("/api/v1/users/{}/tokens?{}", user_id, q)
    };
