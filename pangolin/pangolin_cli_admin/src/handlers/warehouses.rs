...
pub async fn handle_list_warehouses(
    ...
    let path = if q.is_empty() {
        "/api/v1/warehouses".to_string()
    } else {
        format!("/api/v1/warehouses?{}", q)
    };
...
pub async fn handle_create_warehouse(
    ...
    if client.config.tenant_id.is_none() {
        return Err(CliError::ApiError(
            "Root user cannot create warehouses. Please login as Tenant Admin.".to_string(),
        ));
    }
    ...
    for prop in properties {
        if let Some((k, v)) = prop.split_once('=') {
            ...
            println!(
                "Warning: Ignoring invalid property '{}'. format must be key=value",
                prop
            );
        }
    }
    ...
    match type_.as_str() {
        ...
        _ => {
            return Err(CliError::ApiError(format!(
                "Unsupported warehouse type: {}. Use 's3', 'azure', or 'gcs'",
                type_
            )));
        }
    }
...
pub async fn handle_delete_warehouse(
    client: &PangolinClient,
    name: String,
) -> Result<(), CliError> {
    let res = client
        .delete(&format!("/api/v1/warehouses/{}", name))
        .await?;
    if !res.status().is_success() {
...
pub async fn handle_update_warehouse(
    ...
    let res = client
        .put(&format!("/api/v1/warehouses/{}", id), &payload)
        .await?;
