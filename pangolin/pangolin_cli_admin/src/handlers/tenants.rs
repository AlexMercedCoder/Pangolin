...
pub async fn handle_create_tenant(
    ...
    let created_tenant: Value = res
        .json()
        .await
        .map_err(|e| CliError::ApiError(e.to_string()))?;
    let tenant_id = created_tenant["id"]
        .as_str()
        .ok_or(CliError::ApiError("Failed to parse tenant ID".to_string()))?;
    ...
    if let (Some(u), Some(p)) = (admin_username, admin_password) {
        ...
        user_payload.insert(
            "email".to_string(),
            Value::String(format!("{}@example.com", u)),
        );
        user_payload.insert("password".to_string(), Value::String(p));
        user_payload.insert(
            "role".to_string(),
            Value::String("tenant-admin".to_string()),
        );
        user_payload.insert(
            "tenant_id".to_string(),
        ...
        let user_res = client
            .post("/api/v1/users", &Value::Object(user_payload))
            .await?;
        if !user_res.status().is_success() {
            let status = user_res.status();
            let text = user_res.text().await.unwrap_or_default();
            println!(
                "⚠️ Tenant created, but failed to create admin user: {} - {}",
                status, text
            );
        } else {
...
pub async fn handle_update_tenant(
    ...
    let res = client
        .put(&format!("/api/v1/tenants/{}", id), &payload)
        .await?;
