...
pub async fn handle_list_audit_events(
    ...
    if let Some(uid) = user_id {
        query_params.push(format!("user_id={}", uid));
    }
    if let Some(act) = action {
        query_params.push(format!("action={}", act));
    }
    if let Some(rt) = resource_type {
        query_params.push(format!("resource_type={}", rt));
    }
    if let Some(res) = result {
        query_params.push(format!("result={}", res));
    }
    if let Some(tid) = tenant_id {
        query_params.push(format!("tenant_id={}", tid));
    }
    ...
    let res = client
        .get(&format!("/api/v1/audit{}", query_string))
        .await?;
...
pub async fn handle_count_audit_events(
    ...
    let res = client
        .get(&format!("/api/v1/audit/count{}", query_string))
        .await?;
    if !res.status().is_success() {
        return Err(CliError::ApiError(format!("Failed: {}", res.status())));
    }
...
pub async fn handle_get_audit_event(client: &PangolinClient, id: String) -> Result<(), CliError> {
    ...
    println!(
        ...
        event["user_id"].as_str().unwrap_or("None")
    );
    println!("Action:       {}", event["action"].as_str().unwrap_or("-"));
    println!("Result:       {}", event["result"].as_str().unwrap_or("-"));
    println!(
        "Resource:     {} ({})",
        event["resource_name"].as_str().unwrap_or("-"),
        event["resource_type"].as_str().unwrap_or("-")
    );
    println!(
        "Timestamp:    {}",
