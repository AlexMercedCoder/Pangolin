...
pub async fn handle_create_service_user(
    ...
    let id = result["id"]
        .as_str()
        .or_else(|| result["service_user_id"].as_str())
        .unwrap_or("-");
...
pub async fn handle_list_service_users(
    client: &PangolinClient,
    limit: Option<usize>,
    offset: Option<usize>,
) -> Result<(), CliError> {
    ...
    let path = if q.is_empty() {
        "/api/v1/service-users".to_string()
    } else {
        format!("/api/v1/service-users?{}", q)
    };
    let res = client.get(&path).await?;

    if !res.status().is_success() {
        return Err(CliError::ApiError(format!(
            "Failed to list service users: {}",
            res.status()
        )));
    }

    let service_users: Vec<Value> = res
        .json()
        .await
        .map_err(|e| CliError::ApiError(e.to_string()))?;
    ...
    let rows: Vec<Vec<String>> = service_users
        .iter()
        .map(|su| {
            vec![
...
pub async fn handle_get_service_user(client: &PangolinClient, id: String) -> Result<(), CliError> {
    ...
    println!(
        "Active: {}",
        if su["active"].as_bool().unwrap_or(false) {
            "Yes"
        } else {
            "No"
        }
...
pub async fn handle_update_service_user(
    ...
    let res = client
        .put(&format!("/api/v1/service-users/{}", id), &payload)
        .await?;
...
pub async fn handle_delete_service_user(
    client: &PangolinClient,
    id: String,
) -> Result<(), CliError> {
    let res = client
        .delete(&format!("/api/v1/service-users/{}", id))
        .await?;

    if !res.status().is_success() {
        return Err(CliError::ApiError(format!(
            "Failed to delete service user: {}",
            res.status()
        )));
    }
...
pub async fn handle_rotate_service_user_key(
    client: &PangolinClient,
    id: String,
) -> Result<(), CliError> {
    let res = client
        .post(
            &format!("/api/v1/service-users/{}/rotate", id),
            &serde_json::json!({}),
        )
        .await?;
