...
async fn test_resolve_user_id_success() {
    ...
    let alice = users
        .iter()
        .find(|u| u["username"].as_str() == Some("alice"));
    assert!(alice.is_some());
    assert_eq!(
        alice.unwrap()["id"].as_str().unwrap(),
...
async fn test_resolve_user_id_not_found() {
    ...
    let nonexistent = users
        .iter()
        .find(|u| u["username"].as_str() == Some("nonexistent"));
...
async fn test_resolve_role_id_not_found() {
    ...
    let nonexistent = roles
        .iter()
        .find(|r| r["name"].as_str() == Some("nonexistent"));
    assert!(nonexistent.is_none());
}
